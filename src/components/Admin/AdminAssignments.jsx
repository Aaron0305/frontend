...
const AdminAssignments = ({ open, onClose }) => {
    ...
    const getStatusColor = (status, dueDate, closeDate) => {
        // Manejar los nuevos estados específicos del backend
        if (status === 'completed') return 'success';
        if (status === 'completed-late') return 'warning';
        if (status === 'not-delivered') return 'error';
        
        // Para estados pendientes, calcular color según fechas
        if (status === 'pending') {
            const now = new Date();
    ...
    const getStatusLabel = (status, dueDate, closeDate) => {
        ...
        if (status === 'completed-late') return 'Completado con Retraso';
        if (status === 'not-delivered') return 'No Entregado';
        
        // Para estados pendientes, calcular etiqueta según fechas
        if (status === 'pending') {
            const now = new Date();
            const due = new Date(dueDate);
            const close = new Date(closeDate);
                    ...
                    <Grid container spacing={2} sx={{ mb: 3 }}>
                        {[
                            ...
                            { 
                                ...
                                value: stats.overview['completed-late'], 
                                label: 'Con Retraso',
                                color: 'warning',
                            ...
                            { 
                                ...
                                value: stats.overview['not-delivered'], 
                                label: 'No Entregadas',
                                color: 'error',
                        ...
                        <Grid item xs={12} md={2}>
                            <FormControl fullWidth size="small">
                                ...
                                >
                                    ...
                                    <MenuItem value="completed-late">Completadas con Retraso</MenuItem>
                        ...
                        <Table stickyHeader>
                            ...
                            <TableBody>
                                {assignments.map((assignment) => {
                                    // El estado debe venir del backend directamente
